...
import { calculateImageDimensions, Debug, ImageData } from '@felfele/felfele-core';
...
export const resizeImageForPlaceholder = async (image: ImageData, path: string): Promise<string> => {
    const { width, height } = calculateImageDimensions(image, MAX_PLACEHOLDER_DIMENSION, MAX_PLACEHOLDER_DIMENSION);
...
const resizeImage = async (
    ...
    if (isImageExceedMaximumDimensions(image)) {
        const { width, height } = calculateImageDimensions(image, maxDimension, maxDimension);
        const resizedImagePNG = await ImageResizer.createResizedImage(path, width, height, 'PNG', 100);
